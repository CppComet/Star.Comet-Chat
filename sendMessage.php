...
if(isset($_FILES["img"]))
{
    ...
    if (!move_uploaded_file($_FILES["img"]['tmp_name'], "usersFile/".$fileName))
    {
...
if(!mysqli_num_rows($result))
{
    ...
    mysqli_query(app::conf()->getDB(),"INSERT INTO `users_relations` (`user_id`, `to_user_id`, `type`) VALUES ('".$user_id."', '".$to_user_id."', '0'),  ('".$to_user_id."', '".$user_id."', '0');"); 
    ...
    mysqli_query(app::conf()->getDB(),"INSERT INTO `users` (`id`, `login`, `avatar_url`) VALUES ('".$NewContactInfo[0]['user_id']."', '".mysqli_real_escape_string(app::conf()->getDB(),$NewContactInfo[0]['login'])."', '".mysqli_real_escape_string(app::conf()->getDB(),$NewContactInfo[0]['avatar_url'])."');");
    ...
    if($user_id != $to_user_id)
    {
        ...
        mysqli_query(app::conf()->getDB(),
                "INSERT INTO `users` (`id`, `login`, `avatar_url`) VALUES ('".$NewContactInfo[1]['user_id']."', '".mysqli_real_escape_string(app::conf()->getDB(),$NewContactInfo[1]['login'])."', '".mysqli_real_escape_string(app::conf()->getDB(),$NewContactInfo[1]['avatar_url'])."');");
    }
...
{
    $userAgent = mysqli_real_escape_string(app::conf()->getDB(), $_SERVER['HTTP_USER_AGENT']);
}

// Запись в бд
mysqli_query(app::conf()->getDB(), "INSERT INTO `messages` (`id`, `from_user_id`, `to_user_id`, `time`, `read_time`, `message`, `userAgent`)"
        . " VALUES (NULL, '".$user_id."', '".$to_user_id."', '".date("U")."', '0', '".mysqli_real_escape_string(app::conf()->getDB(), $message)."', '".$userAgent."')");
...
$q = mysqli_real_escape_string(app::conf()->getComet(), json_encode($msg));
mysqli_query(app::conf()->getComet(), "INSERT INTO users_messages (id, event, message)VALUES (".$to_user_id.", 'newMessage', '".$q."')");

// Отправка админам 
sendMsgToAdmin("newMessageForUser", $q);
